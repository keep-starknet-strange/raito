...
def request_rpc(method: str, params: list):
    ...
    print(f"Requesting {method} with params {params}")
...
def fetch_block(block_height: int, block_hash: str):
    ...
    utxos = get_utxo_set(block_height + 1)
    block["data"] = {tx["txid"]: resolve_transaction(tx, utxos) for tx in block["tx"]}
    return block
...
def generate_data(
    ...
    for i in range(num_blocks):
        if mode == "light":
            ...
            block = fetch_block(initial_height + i, next_block_hash)
