...
//                     value: 50,
//                     pk_script:
//                     @from_hex("76a914000000000000000000000000000000000000000088ac"), cached:
//                     false,
//                 }
//             ]
//                 .span(),
...
//         let result = validate_transaction(@tx, 0, 1600000000, 1600000000, txid, ref
//         utxo_set);
//         assert!(result.is_err());
//         assert_eq!(
//             result.unwrap_err().into(),
//             "Transaction locktime 1600000000 is not lesser than current block time
//             1600000000"
//         );

    //         utxo_set = Default::default();

    //         // Transaction should be valid when current block time is equal to or greater than
//         locktime let result = validate_transaction(@tx, 0, 1600000001, 1600000001, txid, ref
//         utxo_set);
//         assert!(result.is_ok());
//     }
...
//         let mut utxo_set: UtxoSet = Default::default();
    ...
    //         // Transaction should still valid when current block time is not greater than
//         locktime let result = validate_transaction(@tx, 0, 1600000000, 1600000000, txid, ref
//         utxo_set);
//         assert!(result.is_ok());
...
//         let result = validate_transaction(@tx, 0, 1600000001, 1600000001, txid, ref
//         utxo_set);
//         assert!(result.is_ok());
//     }
...
//         let mut utxo_set: UtxoSet = Default::default();
    ...
    //         // Transaction should still valid when current block time is not greater than
//         locktime let result = validate_transaction(@tx, 500000, 0, 0, txid, ref utxo_set);
//         assert!(result.is_ok());
