...
pub fn log(token_stream: TokenStream) -> ProcMacroResult {
    ...
    let mut args = parsed.descendants(&db).filter(|node| 
        matches!(node.kind(&db), SyntaxKind::Arg)
    );
...
fn create_error_result(message: &str) -> ProcMacroResult {
    ProcMacroResult::new(TokenStream::empty())
        .with_diagnostics(Diagnostic::error(message).into())
}
...
mod tests {
    ...
    fn test_log_macro_debug() {
        ...
        assert_eq!(expanded, r#"if crate::logging::LOG_LEVEL_DEBUG { println!("{}: {}", format!("DEBUG"), format!("test message: {}", '123')) }"#);
    }

    #[test]
    fn test_log_macro_trace() {
        let expanded = expand_macro(log_02lvnd9pht92k, r#"log!("TRACE", "entering function")"#);
        assert_eq!(expanded, r#"if crate::logging::LOG_LEVEL_TRACE { println!("{}: {}", format!("TRACE"), format!("entering function")) }"#);
    }
    ...
    fn test_log_macro_empty_format() {
        ...
        assert_eq!(expanded, r#"if crate::logging::LOG_LEVEL_DEBUG { println!("{}: {}", format!("DEBUG"), format!('')) }"#);
    }

    #[test]
    fn test_log_macro_multiple_arguments() {
        let expanded = expand_macro(log_02lvnd9pht92k, r#"log!("DEBUG", "Values: {}, {}, {}", '1', '2', '3')"#);
        assert_eq!(expanded, r#"if crate::logging::LOG_LEVEL_DEBUG { println!("{}: {}", format!("DEBUG"), format!("Values: {}, {}, {}", '1', '2', '3')) }"#);
    }

    #[test]
    fn test_log_macro_exact_string() {
        let expanded = expand_macro(log_02lvnd9pht92k, r#"log!("DEBUG", "Exact {} string", 'test')"#);
        assert_eq!(expanded, r#"if crate::logging::LOG_LEVEL_DEBUG { println!("{}: {}", format!("DEBUG"), format!("Exact {} string", 'test')) }"#);
    }
}